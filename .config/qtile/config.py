...
class DmenuScreenLayout(extension.Dmenu):
    def __init__(self, **config):
        super().__init__(**config)
        self.add_defaults(extension.Dmenu.defaults)

    def run(self):
        screen_layouts = [
            f[:-3] for f in os.listdir(home + "/.screenlayout") if f.endswith(".sh")
        ]
        choice = super().run(screen_layouts)
        if not choice:
            return
        choice = choice.strip() + ".sh"
...
def dmenu_run_screen_layout():
    return DmenuScreenLayout(
        dmenu_font="Hasklug Nerd Font Mono",
        background=bar_bg,
        foreground=arch_color,
        selected_background=widget_group_color,
...
keys = [
    ...
    Key(
        ["mod1"],
        "space",
        lazy.run_extension(dmenu_run()),
        desc="Spawn DistroTube's dmenu_run",
    ),
    Key(
        [mod],
        "e",
        lazy.spawn(home + "/.config/scripts/edit-dotfiles.sh"),
        desc="Edit dotfiles",
    ),
    Key(
        [mod],
        "g",
        lazy.spawn(home + "/.config/scripts/git-projects.sh"),
        desc="Edit git projects",
    ),
]
...
def init_widgets_list():
    widgets_list = [
        ...
        widget.Bluetooth(
            background=widget_group_color,
            default_show_batter=True,
            device_format='bt {name} {battery_level} [{symbol}]'
        ),
        widget.Volume(
            background=widget_group_color,
            fmt="v {}",
        ...
        widget.Volume(
            ...
            foreground=term_colors[2],
            device="Capture",
            get_volume_command="amixer -D default sget Capture | grep Mono: | awk -F'[][]' '{ print $2 }'",
            volume_up_command="amixer -D default sget Capture | grep Mono: | awk -F'[][]' '{ print $2 }' | tr -d '%' | awk '{print $1 + 1\"%\"}' | xargs amixer -D default sset Capture",
            volume_down_command="amixer -D default sget Capture | grep Mono: | awk -F'[][]' '{ print $2 }' | tr -d '%' | awk '{print $1 - 1\"%\"}' | xargs amixer -D default sset Capture",
        ),
        widget.Backlight(
            background=widget_group_color,
            foreground=term_colors[2],
            ...
            fmt="b {}",
            change_command="sudo xbacklight -set {}",
        ),
        widget.TextBox(
            foreground=widget_group_color,
            text=widget_lsep + " ",
